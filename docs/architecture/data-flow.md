...
## Phase 2 durable upload agent - contract pending independent review

[P2-UPLOAD-v1 revision 2](phase-2-upload-contract.md) specifies producer marker-last publication, atomic per-item queue state, single-process lock, strict acknowledgment/retry, per-attempt object ownership, fenced DB completion and exclusive receipt-aware maintenance. It supersedes this former outline only; accepted manual Phase 1 flow remains unchanged.
