...
out=root/".orchestration/reports/BACKEND-OCR-001-linux-capture-outer-06"
script=root/".orchestration/reports/BACKEND-OCR-001-native-apply-linux-capture-03.py"
pins=root/".orchestration/reports/BACKEND-OCR-001-linux-source-pins-05.json"
evidence=root/".orchestration/reports/BACKEND-OCR-001-native-apply-linux-evidence-03-06"
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def write(name,data):
    with (out/name).open("x",encoding="utf-8",newline="\n") as f:f.write(json.dumps(data,indent=2)+"\n")
assert sys.flags.optimize==0 and not evidence.exists()
assert sha(script)=="d0a8e756cdd4cafc394d12a68e527632365aeaf43f6d9ca8cc063763db21d7fd"
